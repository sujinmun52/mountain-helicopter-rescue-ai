...
조난자 GPS 입력
  ...
  ② Dynamic  기상청 실시간 API(현재 −20분) → 풍속·풍향으로 적합도 재평가
  ③ ML       XGBoost 4모드 분류 → 착륙지별 위험등급(안전/주의/위험) + 적합도 산출
             → 착륙(A안)·호이스트(B안) 각 1·2·3순위(50m 이격) 제시
  ④ A*       비행(119→구조점) + 도보(구조점→조난자) 최적 경로 산출
...
| Dynamic | 기상청 실시간 API | 풍속·풍향 + 고도보정 | 동적 적합도 점수 |
| ML | 13개 피처(지형+기상+상호작용) | XGBoost 4모드 추론 | 위험등급·적합도·1·2·3순위 |
| A* | 적합도·지형·바람 | 비행=풍속 회피, 도보=지형 회피 | 최적 경로·ETA |
...
- 두 가지 출력
  - **위험등급(risk scoring, `risk_class`)**: 작전 가능성 3분류
  ...
  - **적합도(`suitability_score`)**: 연속 점수(높을수록 안전/적합). 동일 등급 내 순위 정렬에 사용.
    위험도가 아니라 적합도이며, 화면에는 "적합도"로 표기.
- 후보 선정: 등급(오름차순) → 적합도(내림차순) → 거리(오름차순)로 정렬 후, 50m 이격(공간 다각화)으로 1·2·3순위 선정
...
│   ├── rescue_zone_inference.py  #   [Stage 2] XGBoost 추론 — 착륙/호이스트 1·2·3순위 선정
│   ├── rescue_zone.py            #   [Stage 2] 구조구역 선정 셸 (추론 호출 + 휴리스틱 폴백)
│   ├── risk_scoring.py           #   헬기 4모드 적합도 스코어링
...
### 웹앱(`app.py`) 기능
- 조난자 GPS + 기종 입력 → 착륙(A안)·호이스트(B안) **각 1·2·3순위** 추천 (등급·적합도·거리, 기장 판단)
- 전술 선택 → A* 경로(1순위 기준) 산출 + 착륙지·조난자 상세(경사·풍속·임상 등) + ETA
- **2D 미션맵**: 실제 지도 + 경로 우회 이유(hover) + 실시간 기상 연동 배지 + 2·3순위 레이어 토글 + 좌측 스크롤 브리핑
- **3D 지형뷰**: 회전/확대 + 드래그=회전↔지도이동(pan) 토글 버튼 + 2·3순위 표시 토글 + 고정·드래그 정보패널("2·3순위인 이유" 펼침)
...
- 구조구역 선정 통합 — `rescue_zone.py`가 실 XGBoost 추론(착륙/호이스트 1·2·3순위) 호출, 모델 부재 시에만 휴리스틱 폴백
- 실시간 기상 융합 — 기상청 AWS 관측 + 고도보정으로 동적 적합도 산출
...
- A* 휴리스틱에 적합도/위험 완전 통합 — λ 가중치 튜닝으로 위험도-거리 균형 최적화
