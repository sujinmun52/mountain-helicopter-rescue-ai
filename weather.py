...
def safe_kma_get(url, params, max_retries=3, timeout=15, sleep_sec=1.5):
    """
    KMA API 안정 호출 함수
    - 일시적 네트워크 오류/타임아웃 발생 시 재시도
    - 마지막까지 실패하면 None 반환
    """
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            print(f"[KMA API] attempt={attempt}, status={response.status_code}")
            ...
            if not text or text.strip() == "":
                print("[KMA API 경고] 빈 응답 수신")
                raise ValueError("Empty response from KMA API")

            return text

        except requests.exceptions.Timeout:
            print(f"[KMA API 타임아웃] {attempt}/{max_retries}회 실패")

        except requests.exceptions.ConnectionError as e:
            print(f"[KMA API 연결 오류] {attempt}/{max_retries}회 실패: {e}")

        except requests.exceptions.HTTPError as e:
            print(f"[KMA API HTTP 오류] {attempt}/{max_retries}회 실패: {e}")
            print(f"응답 내용 일부: {response.text[:300] if response is not None else 'No response'}")

        except Exception as e:
            print(f"[KMA API 기타 오류] {attempt}/{max_retries}회 실패: {e}")

        if attempt < max_retries:
            time.sleep(sleep_sec * attempt)

    print("[KMA API 최종 실패] 모든 재시도 실패")
    return None

if __name__ == "__main__":
    print("[TEST] weather.py 직접 실행")
    print(f"[TEST] KMA_API_KEY loaded: {KMA_API_KEY[:4]}***")

    data = fetch_kma_realtime()
