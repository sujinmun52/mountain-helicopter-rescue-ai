...
def run(terrain_df: pd.DataFrame) -> list[str]:
    ...
    # 하드 블록 조건: 경사도 45도 이상 OR 수관밀도 2 이상(70%+ 밀도)
    ...
    # 안전/주의 지형은 세밀하게, 이미 위험한 지형은 극단값만 주입
    ...
    # 풍향은 0~359도 사이의 난수를 발생시켜 다양한 진입 방향을 학습하도록 유도
    np.random.seed(42)
    ...
    # 조난자 중심 좌표 대신 랜덤한 가상의 비행 방향(flight_heading)을 생성하여 풍향 스코어 계산
    ...
    # ── [STEP 4] 타겟 라벨링 및 저장 ──
    # 주의: score_utils.py의 compute_targets 내부에서 풍속 10m/s(소형), 20m/s(대형) 
    # 제약이 반영되도록 로직이 수정되어 있어야 합니다.
    try:
