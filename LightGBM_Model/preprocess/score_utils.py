...
# 전술별 (정적 가중치, 동적 가중치) 분리 정의 (LightGBM 공식 반영)
TACTIC_WEIGHTS = {
    "small_landing": {
...
FEATURE_COLUMNS = [
    ...
    # 원-핫 인코딩된 토양 변수들 (LightGBM 데이터셋 기준에 맞춰 매핑)
    'land_0', 'land_1', 'land_2' 
]

TARGET_COLUMNS = [
    'target_small_landing', 'target_large_landing',
    'target_small_hoist', 'target_large_hoist',
]
...
def compute_targets(df):
    """정적 성분 + 동적 성분 브로드캐스팅 → 최종 점수 및 LightGBM용 3클래스 라벨."""
    for key, w in TACTIC_WEIGHTS.items():
        d = w["dynamic"]
        score = (
            df[f'static_score_{key}']
