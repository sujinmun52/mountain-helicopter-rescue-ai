...
print("="*60)
print(" 1️⃣ 소형 안착 착륙 모델 (Small Helicopter Landing)")
print(" 2️⃣ 소형 강하 호이스트 모델 (Small Helicopter Hoist)")
print(" 3️⃣ 대형 안착 착륙 모델 (Large Helicopter Landing)")
print(" 4️⃣ 대형 강하 호이스트 모델 (Large Helicopter Hoist)")
print("="*60)
...
print("💡 만약 중간에 뻑나더라도 이 모델 외의 다른 가중치 파일은 안전하게 보존됩니다.\n")
...
print(f"   ➡️ 훈련셋 데이터: {len(X_train)}개 | 검증셋 데이터: {len(X_test)}개")
...
rf_model = RandomForestClassifier(
    ...
    n_jobs=-1, 
    random_state=42,
    max_samples=4000000  # 🎯 메모리 튕김 차단용 하위 샘플링 버퍼
)
...
print(classification_report(test_df[target_column], rf_model.predict(X_test)))

# 전용 독립 파일 매핑 저장
output_model_path = f'RF_Model\\rf_{model_key}_model.joblib'
joblib.dump(rf_model, output_model_path)

print(f"\n🎉 【 {model_kor_name} 】 단독 학습 및 직렬화 안전 완료! ➡️ {output_model_path}")
print("==============================================================================")