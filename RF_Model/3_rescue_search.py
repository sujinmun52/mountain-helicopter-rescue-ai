...
# 데이터셋 연동 동기화: 랜덤 포레스트 전용 마스터 데이터셋(_rf.csv)을 로드합니다.
df_master = pd.read_csv(r'RF_Model\dataset\processed_seoraksan_master_rf.csv')
...
def find_best_rescue_tactics(rescue_lat, rescue_lon, heli_type, search_radius_meters=500):
    ...
    # 반경 필터: 실시간 탐색 여부와 상관없이 설정한 search_radius_meters 범위 내 격자만 남김
    candidates = candidates[candidates['dist_to_rescue'] * 110000 <= search_radius_meters].copy()

    # 하천 구역 배제 예외 규칙 전역 적용
    # 착륙 및 호이스트 공통으로 land_type이 2(land_2 == 1)인 하천 격자를 전면 격리 차단합니다.
